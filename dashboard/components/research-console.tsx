...
export type ResearchSnapshot = {
  schema: string;
  campaign: string;
  status: { model: string; reasoning_effort: string; verifier_reasoning_effort?: string; verifier_count?: number; fixed_prime: number; fixed_degree: number; recovery_divisor: number; minimum_soundness?: string; minimum_recovery_agreement_count?: number; researcher_count: number; planned_agent_invocations: number; counts: Record<string, number>; updated_at: string };
...
function RecordChart({ points }: { points: HistoryPoint[] }) {
  ...
  return <div className="chart-wrap">
    ...
    {!points.length && <div className="chart-empty"><ShieldCheck /><strong>No promoted result yet</strong><span>The chart accepts only exact claims approved by one independent xhigh verifier.</span></div>}
  </div>;
}
...
export function ResearchConsole({ initialData }: { initialData: ResearchSnapshot }) {
  const [data, setData] = useState(initialData), [view, setView] = useState<View>('record'), [query, setQuery] = useState(''), [refreshing, setRefreshing] = useState(false), [syncState, setSyncState] = useState<'idle' | 'success' | 'error'>('idle'), [copied, setCopied] = useState(false), [submissionMessage, setSubmissionMessage] = useState(''), [submissionType, setSubmissionType] = useState<'research' | 'leaderboard'>('research');
  async function refresh() { setRefreshing(true); setSyncState('idle'); try { const response = await fetch(`./research-data.json?t=${Date.now()}`, { cache: 'no-store' }); if (!response.ok) throw new Error(`refresh failed: ${response.status}`); setData(await response.json() as ResearchSnapshot); setSyncState('success'); window.setTimeout(() => setSyncState('idle'), 2200); } catch { setSyncState('error'); } finally { setRefreshing(false); } }
  ...
  async function stageExternalSubmission(event: unknown) {
    (event as { preventDefault: () => void }).preventDefault();
    const form = new FormData((event as { currentTarget: HTMLFormElement }).currentTarget);
    const file = form.get('document') as File | null;
    const contributionEntry = form.get('contribution_type');
    const contributionType = typeof contributionEntry === 'string' ? contributionEntry : '';
    if (!file || !/\.(md|tex|pdf)$/i.test(file.name)) { setSubmissionMessage('Choose a .md, .tex, or .pdf document.'); return; }
    let agreements: number | null = null;
    if (contributionType === 'leaderboard') {
      agreements = Number(form.get('agreements'));
      if (!Number.isSafeInteger(agreements) || agreements <= 0 || agreements > 147457 * 147457) { setSubmissionMessage('A leaderboard submission needs an integer score between 1 and p².'); return; }
    }
    ...
    const authorEntry = form.get('author');
    const title = typeof titleEntry === 'string' ? titleEntry.trim() : '';
    const slug = title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 60) || 'line-point-contribution';
    const manifest = {
      schema: 'line-point-external-submission-v2', contribution_type: contributionType,
      title, author: typeof authorEntry === 'string' ? authorEntry.trim() : '', filename: file.name,
      absolute_agreements: agreements, normalized_epsilon: agreements == null ? null : agreements / (147457 * 147457),
      fixed_prime: 147457, fixed_degree: 87, dimension: 2, packaged_at: new Date().toISOString(),
    };
    const bundle = { schema: 'line-point-handoff-bundle-v1', manifest, document: { filename: file.name, media_type: file.type || 'application/octet-stream', content_base64: contentBase64 } };
    const blob = new Blob([JSON.stringify(bundle, null, 2) + '\n'], { type: 'application/json' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob); link.download = `${slug}.lvp-submission.json`; link.click(); URL.revokeObjectURL(link.href);
    setSubmissionMessage('Handoff bundle downloaded. Nothing was uploaded: return this one file to the person who sent you here.');
  }
  ...
  useEffect(() => { const timer = window.setInterval(() => { void refresh(); }, 60_000); const onVisible = () => { if (document.visibilityState === 'visible') void refresh(); }; document.addEventListener('visibilitychange', onVisible); return () => { window.clearInterval(timer); document.removeEventListener('visibilitychange', onVisible); }; }, []);
  ...
  </nav><button className="sync" onClick={() => void refresh()} disabled={refreshing} aria-live="polite"><RefreshCw className={refreshing ? 'spin' : ''} /> {refreshing ? 'Syncing' : syncState === 'success' ? 'Synced' : syncState === 'error' ? 'Retry sync' : 'Sync'}</button></header>
  ...
  {view === 'record' && <div className="page"><section className="hero"><div><p className="kicker">Concrete line-vs-point test</p><h1>How few agreement points can be certified?</h1><p className="lede">For <MathText className="inline">{'$\\varepsilon\\ge 1.1d/p$'}</MathText>, acceptance at least <MathText className="inline">{'$\\varepsilon$'}</MathText> must force a total-degree-87 polynomial agreeing on at least <MathText className="inline">{'$\\max\\{2d/p,\\varepsilon/10\\}$'}</MathText> of the points.</p></div><div className="record-number"><small>best verified agreement count</small><strong>{formatCount(best?.agreement_count)}</strong><span>ε = {formatEpsilon(best?.soundness)} · lower is better ↓</span></div></section>
  ...
  {view === 'contribute' && <div className="page contribute-page"><section className="contribute-hero"><div><p className="kicker">Open mathematical collaboration</p><h1>Try to move the record.</h1><p>Bring a complete soundness proof, sharpen one KTZ loss, propose a reusable lemma, or attack the current architecture. Every durable contribution is preserved; only end-to-end leaderboard claims consume verifier audits.</p><div className="contribute-actions"><button className="copy-prompt" onClick={copyContributorPrompt}>{copied ? <CheckCheck /> : <Copy />}{copied ? 'Copied full prompt' : 'Copy research prompt'}</button><a href="https://github.com/kz99/line-vs-point-concrete/tree/main/how_to_contribute" target="_blank" rel="noreferrer">Read full guide <ExternalLink /></a><a href="https://github.com/kz99/line-vs-point-concrete/compare" target="_blank" rel="noreferrer">Open a pull request <GitPullRequest /></a></div></div><aside><span>fixed target</span><MathText>{'$$p=147457,\\qquad d=87,\\qquad m=2$$'}</MathText><MathText>{'$$\\varepsilon\\ge 1.1d/p,\\qquad \\operatorname{Pass}(f,P)\\ge\\varepsilon\\;\\Longrightarrow\\;\\operatorname{Agr}_{87}(f)\\ge\\max\\{2d/p,\\varepsilon/10\\}$$'}</MathText><strong>lower verified ε is the objective</strong></aside></section>

  <section className="contribute-grid"><article className="contribute-card steps"><p className="kicker">Submission path</p><h2>From idea to public audit</h2><ol><li><span>01</span><div><strong>Clone or keep working</strong><p>Clone the public repository when possible. An empty workspace never blocks the mathematical work.</p></div></li><li><span>02</span><div><strong>Write the mathematical note</strong><p>State exact hypotheses, number the proof chain, cite primary results, and expose every numerical loss.</p></div></li><li><span>03</span><div><strong>Validate when possible</strong><p><code>PYTHONPATH=src python -m line_point_research community-validate community_submissions/&lt;slug&gt;</code></p></div></li><li><span>04</span><div><strong>Hand it off</strong><p>Open a pull request, or package the note in the Submit tab and return the bundle to the person who assigned the research.</p></div></li></ol></article>

  <article className="contribute-card tracks"><p className="kicker">Choose honestly</p><h2>Two contribution tracks</h2><div><ShieldCheck /><section><strong>Leaderboard submission</strong><p>A proved numerical end-to-end theorem. One independent xhigh auditor traverses its complete logic chain and verifies every lemma it uses. One accept is required for the graph.</p></section></div><div><BookOpen /><section><strong>Research contribution</strong><p>A lemma, proof tool, obstruction, counterexample, or conditional architecture. It enters the shared corpus without audit and is verified only if a later leaderboard proof depends on it.</p></section></div></article></section>
  ...
  {view === 'submit' && <div className="page narrow"><section className="section-intro"><Upload /><div><p className="kicker">Portable handoff</p><h1>Package a contribution</h1><p>No GitHub account is required. Create one portable file and return it in the conversation or workspace from which you were sent here.</p></div></section><section className="contribute-card external-submit"><div className="absolute-score"><p className="kicker">Leaderboard submissions only</p><h2>Absolute soundness score</h2><MathText>{'$$A=\\left\\lceil\\varepsilon p^2\\right\\rceil$$'}</MathText><p>A complete end-to-end theorem must state the integer score at its top. A standalone lemma or partial result needs no score.</p></div><form onSubmit={stageExternalSubmission}><label>Contribution track<select name="contribution_type" value={submissionType} onChange={(event) => setSubmissionType(event.target.value as 'research' | 'leaderboard')}><option value="research">Research contribution</option><option value="leaderboard">Leaderboard submission</option></select></label>{submissionType === 'leaderboard' && <label>Absolute soundness score (A)<input name="agreements" type="number" min="1" max={147457 * 147457} required placeholder={`must be below ${formatCount(best?.agreement_count)}`} /></label>}<label>Submission title<input name="title" required placeholder="A concise theorem or lemma title" /></label><label>Author or handle<input name="author" required placeholder="Your name or pseudonym" /></label><label>Mathematical note (.md, .tex, or .pdf)<input name="document" type="file" accept=".md,.tex,.pdf" required /></label><button className="copy-prompt" type="submit"><Download /> Download one-file handoff bundle</button></form>{submissionMessage && <p className="submit-message">{submissionMessage}</p>}<p className="submit-note"><strong>This page does not upload files.</strong> GitHub Pages is a static host. It creates a self-contained <code>.lvp-submission.json</code> bundle in your browser. Return that one file to the person who assigned the research; the project operator can ingest it into <code>external-submission/</code>. This replaces the nonexistent “external inbox” previously mentioned here.</p></section></div>}
