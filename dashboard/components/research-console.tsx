...
export function ResearchConsole({ initialData }: { initialData: ResearchSnapshot }) {
  const [data, setData] = useState(initialData), [view, setView] = useState<View>('record'), [query, setQuery] = useState(''), [refreshing, setRefreshing] = useState(false), [syncState, setSyncState] = useState<'idle' | 'success' | 'error'>('idle'), [copied, setCopied] = useState(false), [submissionMessage, setSubmissionMessage] = useState('');
